...
class GenerationContext:
    scenario_map: dict[pd.Timestamp, dict[str, float | str]]
    impact_lookup: dict[tuple[pd.Timestamp, str], float]
    version_catalog: VersionCatalog
    version_map: dict[str, dict[str, object]]
    markets: list[str]
    market_p: np.ndarray
    planned_hours_market: dict[str, tuple[int, int]]
    turnos_df: pd.DataFrame


@dataclass
class OrderVehicleFrames:
    ordenes: pd.DataFrame
    vehiculos: pd.DataFrame


@dataclass
class ChargingFrames:
    vehiculos: pd.DataFrame
...
def _deduplicate_patio_points(
    ...
    selected: dict[pd.Timestamp, tuple[pd.Timestamp, str, str]] = {}
    for point in points:
        timestamp, zone, state = point
        current = selected.get(timestamp)
        candidate_key = (state_priority.get(state, -1), state, zone)
        if current is None:
            selected[timestamp] = point
            continue
        current_key = (state_priority.get(current[2], -1), current[2], current[1])
...
def _active_charging_slots(slots_carga: pd.DataFrame) -> pd.DataFrame:
    active_slots = slots_carga.loc[
        slots_carga["disponibilidad_flag"].eq(1) & slots_carga["mantenimiento_flag"].eq(0)
    ].copy()
    if active_slots.empty:
        raise ValueError("slots_carga debe contener al menos un punto disponible y fuera de mantenimiento.")
    return active_slots
...
def _prepare_generation_context(
    ...
    return GenerationContext(
        ...
        markets=markets,
        market_p=market_p,
        planned_hours_market=planned_hours_market,
        turnos_df=turnos_df,
    )


def _generate_orders_and_vehicles(
    ...
    for shift_row in context.turnos_df.itertuples(index=False):
        scn = context.scenario_map[pd.Timestamp(shift_row.fecha)]

        prod_impact = context.impact_lookup.get((pd.Timestamp(shift_row.fecha), "PRODUCCION"), 0.0)
        patio_impact = context.impact_lookup.get((pd.Timestamp(shift_row.fecha), "PATIO"), 0.0)
        ...
        for seq in range(1, n_orders + 1):
            version_id = _select_version(rng, context.version_catalog, scn["share_ev"])
            version = context.version_map[version_id]
            ...
            mercado = str(rng.choice(context.markets, p=context.market_p))
    ...
    return OrderVehicleFrames(ordenes=ordenes_df, vehiculos=vehiculos_df)


def _generate_charging_records(
    rng: np.random.Generator,
    context: GenerationContext,
    slots_carga: pd.DataFrame,
    frames: OrderVehicleFrames,
) -> ChargingFrames:
    ordenes_df = frames.ordenes
    vehiculos_df = frames.vehiculos.copy()
    active_slots = _active_charging_slots(slots_carga)
    ...
    ev_vehicle_idx = vehiculos_df[
        vehiculos_df["version_id"].map(lambda x: int(context.version_map[x]["requiere_carga_salida_flag"]) == 1)
    ...
    for v in ev_vehicle_idx.itertuples(index=True):
        ...
        scn = context.scenario_map.get(v_date, list(context.scenario_map.values())[-1])

        charge_impact = context.impact_lookup.get((v_date, "CARGA"), 0.0)
        patio_impact = context.impact_lookup.get((v_date, "PATIO"), 0.0)
        ...
        version = context.version_map[v.version_id]
    ...
    return ChargingFrames(
        ...
        sesiones=pd.DataFrame(sessions_rows, columns=SESSION_COLUMNS).sort_values("inicio_sesion"),
        estado_bateria=pd.DataFrame(battery_rows, columns=BATTERY_COLUMNS).sort_values(["timestamp", "vehiculo_id"]),
    )


def _generate_logistics_yard_and_outputs(
    rng: np.random.Generator,
    context: GenerationContext,
    slots_carga: pd.DataFrame,
    order_frames: OrderVehicleFrames,
    charging_frames: ChargingFrames,
) -> OperationalOutputs:
    ordenes_df = order_frames.ordenes
    vehiculos_df = charging_frames.vehiculos.copy()
    sesiones_df = charging_frames.sesiones
    estado_bateria_df = charging_frames.estado_bateria
    ...
    for veh in vehiculos_df.itertuples(index=True):
        ...
        logistic_impact = context.impact_lookup.get((v_date, "LOGISTICA"), 0.0)
        patio_impact = context.impact_lookup.get((v_date, "PATIO"), 0.0)

        low_h, high_h = context.planned_hours_market[str(veh.mercado_destino)]
        ...
        readiness_initial = bool(readiness_batt and (veh.readiness_score_inicial >= 28))
        readiness_on_plan = int(readiness_initial and base_ready <= fecha_planificada)
        if readiness_on_plan:
            timestamp_readiness = base_ready
        else:
            remediation_base = 420 if is_ev_charge_flow else 240
            remediation_minutes = int(
                max(
                    60,
                    rng.normal(
                        remediation_base + 240 * logistic_impact + 180 * patio_impact,
                        90,
                    ),
                )
            )
            timestamp_readiness = max(base_ready, fecha_planificada) + pd.Timedelta(minutes=remediation_minutes)
        ...
        extra_delay = max(
            ...
            int(
                rng.normal(
                    ...
                    + (90 if not readiness_on_plan else 0),
                    60,
                )
            ),
        )

        fecha_real_salida = max(fecha_planificada, timestamp_readiness) + pd.Timedelta(minutes=extra_delay)

        if fecha_real_salida > last_reference_ts:
        ...
        if not readiness_on_plan:
            causa = "READINESS_PENDIENTE" if pd.isna(fecha_real_salida) else "READINESS_RECUPERADA_CON_RETRASO"
        elif retraso <= 20:
            causa = "SIN_RETRASO"
        ...
        logistics_rows.append(
            {
                ...
                "timestamp_readiness": timestamp_readiness,
                "modo_salida": modo,
                "transportista_proxy": str(rng.choice(["Carrier_A", "Carrier_B", "Carrier_C", "Carrier_D"])),
                "readiness_salida_flag": readiness_on_plan,
                "retraso_min": retraso,
                "causa_retraso": causa,
            }
        )
        ...
        elif retraso > 420 or readiness_on_plan == 0:
        ...
        if pd.notna(veh.timestamp_inicio_carga):
            ...
            ts_charge_end = pd.Timestamp(veh.timestamp_fin_carga)
            patio_points.append((ts_charge_end, "BUFFER_CARGA", "POST_CARGA"))
            patio_points.append(
                (
                    ts_charge_end + pd.Timedelta(minutes=int(rng.integers(8, 20))),
                    zone_entry,
                    "EN_ESPERA_SALIDA",
                )
            )
...
def generate_operational_tables(
    ...
    context = _prepare_generation_context(
        ...
        versiones=versiones,
        slots_carga=slots_carga,
        daily_restriction_map=daily_restriction_map,
    )
    order_frames = _generate_orders_and_vehicles(rng, context)
    charging_frames = _generate_charging_records(rng, context, slots_carga, order_frames)
    return _generate_logistics_yard_and_outputs(
        rng,
        context,
        slots_carga,
        order_frames,
