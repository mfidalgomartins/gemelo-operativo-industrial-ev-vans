...
planned AS (
    SELECT
        ev_operational_date(d.fecha_salida_planificada) AS fecha,
        ev_operational_shift(d.fecha_salida_planificada) AS turno,
        COUNT(d.salida_id) AS dispatch_plan
    FROM stg_dispatch d
    GROUP BY
        ev_operational_date(d.fecha_salida_planificada),
        ev_operational_shift(d.fecha_salida_planificada)
),
actual AS (
    SELECT
        ev_operational_date(d.fecha_salida_real) AS fecha,
        ev_operational_shift(d.fecha_salida_real) AS turno,
        COUNT(d.salida_id) AS dispatch_actual
    FROM stg_dispatch d
    WHERE d.fecha_salida_real IS NOT NULL
        GROUP BY
        ev_operational_date(d.fecha_salida_real),
        ev_operational_shift(d.fecha_salida_real)
),
yard_shift AS (
    SELECT
