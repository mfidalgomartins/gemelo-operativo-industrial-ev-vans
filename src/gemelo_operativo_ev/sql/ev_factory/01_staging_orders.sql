...
-- Contrato temporal común: el turno C empieza a las 22:00 y las marcas
-- comprendidas entre 00:00 y 05:59 pertenecen a la fecha operativa anterior.
CREATE OR REPLACE MACRO ev_operational_shift(event_ts) AS
    CASE
        WHEN event_ts IS NULL THEN NULL
        WHEN EXTRACT('hour' FROM event_ts) BETWEEN 6 AND 13 THEN 'A'
        WHEN EXTRACT('hour' FROM event_ts) BETWEEN 14 AND 21 THEN 'B'
        ELSE 'C'
    END;

CREATE OR REPLACE MACRO ev_operational_date(event_ts) AS
    CASE
        WHEN event_ts IS NULL THEN NULL
        WHEN EXTRACT('hour' FROM event_ts) < 6 THEN CAST(event_ts - INTERVAL 1 DAY AS DATE)
        ELSE CAST(event_ts AS DATE)
    END;
...
WITH typed_orders AS (
    SELECT
        ...
        CAST(fecha_turno_operativo AS DATE) AS fecha_turno_operativo,
