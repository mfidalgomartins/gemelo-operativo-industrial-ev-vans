...
from ..config import DATA_PROCESSED_DIR, OUTPUT_GRAPHS_DIR, PROJECT_ROOT

# ── Rutas
ROOT = PROJECT_ROOT
DATA = DATA_PROCESSED_DIR / "ev_factory"
OUT = OUTPUT_GRAPHS_DIR
...
# Un día en el borde de la cobertura se considera parcial cuando su volumen
# queda por debajo del 50 % de la mediana diaria del corte. La regla solo recorta
# los extremos: conserva días internos de bajo volumen porque pueden ser señal
# operativa real, no falta de cobertura.
PARTIAL_DAY_MIN_COVERAGE_RATIO = 0.50
...
def scenario_label(value: str) -> str:
    """Convierte el identificador técnico de escenario en una etiqueta visible."""
    key = str(value)
    return SCENARIO_NAME_ES.get(key, key.replace("_", " ").strip().capitalize())


def prepare_daily_throughput(
    frame: pd.DataFrame,
    min_coverage_ratio: float = PARTIAL_DAY_MIN_COVERAGE_RATIO,
) -> pd.DataFrame:
    """Agrega caudal diario y elimina únicamente bordes con cobertura parcial.

    La cobertura relativa es el volumen único del día dividido por la mediana
    de los días internos del corte, que no pueden estar truncados por el límite
    temporal. Se eliminan los días contiguos incompletos al inicio o al final;
    una caída interna se conserva como observación operativa.
    """
    required = {"fecha_real", "vehiculo_id"}
    missing = required.difference(frame.columns)
    if missing:
        raise ValueError(f"Faltan columnas requeridas: {', '.join(sorted(missing))}")
    if not 0 < min_coverage_ratio <= 1:
        raise ValueError("min_coverage_ratio debe estar en el intervalo (0, 1]")

    clean = frame.loc[:, ["fecha_real", "vehiculo_id"]].copy()
    clean["fecha_real"] = pd.to_datetime(clean["fecha_real"], errors="coerce")
    clean = clean.dropna(subset=["fecha_real", "vehiculo_id"])
    if clean.empty:
        raise ValueError("No hay observaciones válidas para calcular el caudal diario")

    daily = (
        clean.assign(fecha=clean["fecha_real"].dt.normalize())
        .groupby("fecha", as_index=False)
        .agg(real=("vehiculo_id", "nunique"))
        .sort_values("fecha")
        .reset_index(drop=True)
    )
    # Los extremos son los únicos candidatos a cobertura parcial. Cuando hay
    # suficientes días, se excluyen solo del cálculo de la referencia para que
    # un borde truncado no deprima la propia mediana contra la que se compara.
    internal_mask = ~daily.index.isin([daily.index[0], daily.index[-1]])
    reference_volume = daily.loc[internal_mask, "real"] if internal_mask.any() else daily["real"]
    median_volume = float(reference_volume.median())
    if median_volume <= 0:
        raise ValueError("La mediana diaria debe ser positiva")

    daily["coverage_ratio"] = daily["real"] / median_volume
    complete_edge = daily["coverage_ratio"] >= min_coverage_ratio
    if not complete_edge.any():
        raise ValueError("Ningún día alcanza la cobertura mínima")

    first_complete = int(complete_edge[complete_edge].index[0])
    last_complete = int(complete_edge[complete_edge].index[-1])
    return daily.loc[first_complete:last_complete].reset_index(drop=True)


def format_period_es(dates: pd.Series) -> str:
    """Devuelve el período mensual observado sin fechas hardcodeadas."""
    parsed = pd.to_datetime(dates, errors="coerce").dropna()
    if parsed.empty:
        raise ValueError("No hay fechas válidas para describir el período")

    start = parsed.min()
    end = parsed.max()
    start_label = f"{MONTH_NAME_ES[start.month]} de {start.year}"
    end_label = f"{MONTH_NAME_ES[end.month]} de {end.year}"
    return start_label if (start.year, start.month) == (end.year, end.month) else f"{start_label} a {end_label}"


# ──────────────────────────────────────────────────────────────────────────────
def chart_01_throughput() -> None:
    df = pd.read_csv(DATA / "vw_vehicle_flow_timeline.csv", parse_dates=["fecha_real"])
    daily = prepare_daily_throughput(df)
    daily["real_7d"] = daily["real"].rolling(7, min_periods=1).mean()
    avg = daily["real"].mean()
    period_label = format_period_es(daily["fecha"])
    ...
    footer(fig, f"Fuente: vw_vehicle_flow_timeline · período {period_label}")
...
def chart_03_priority_ranking() -> None:
    ...
    ax.set_xlabel("Índice de Prioridad Operativa (OPI)")
    ax.set_ylabel("")
    ax.spines["left"].set_visible(False)
    ax.spines["bottom"].set_color(LINE)
...
def chart_05_scenario_decision() -> None:
    ...
    df["label"] = df["escenario"].map(scenario_label)
...
def chart_19_before_after() -> None:
    ...
    ax.set_xlim(-0.2, 1.2)
    ax.set_xticks([0, 1])
    ax.set_xticklabels(["Rampa EV base", "Combinación correctiva"], fontsize=10, color=INK)
    ax.tick_params(length=0)
    for sp in ax.spines.values():
        sp.set_visible(False)
    ax.grid(False)
